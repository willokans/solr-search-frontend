...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  let solrUrl = 'http://localhost:8983/solr/corename/select?indent=true&q.op=OR&q=*%3A*';

  // If searching for company_name, build a query
  if (company_name && typeof company_name === 'string' && company_name.length > 0) {
    solrUrl = `http://localhost:8983/solr/corename/select?indent=true&q.op=OR&q=company_name:${encodeURIComponent(company_name)}*`;
  }
